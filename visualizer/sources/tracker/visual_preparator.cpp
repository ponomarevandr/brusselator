...
void VisualPreparator::addArrow(SegmentedLine& track) {
	Point current = track.back();
	Point previous = track[track.size() - 2];
	...
	track.push_back(left_wing);
	track.push_back(current);
	track.push_back(right_wing);
	track.push_back(current);
}
